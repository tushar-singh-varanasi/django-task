...
class UserSerilizer(serializers.ModelSerializer):
    ...
    # user_levels=serializers.CharField(source='get_user_levels_display')
    class Meta:
        model=User
...
class SendPasswordResetserializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if User.objects.filter(email=email).exists():
            ...
            print('password reser token',token)
            link='http://localhost:3000/reset-password/'+uid+'/'+token
            print('password reser link',link)
            # send email
...
class UserPasswordReseSerializer(serializers.Serializer):
    password=serializers.CharField(max_length=200,style={'input_type':'password'},write_only=True)
    password2=serializers.CharField(max_length=200,style={'input_type':'password'},write_only=True)
    class Meta:
        fields=['password','password2']
